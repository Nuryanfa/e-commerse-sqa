    ...
    <AnimatePresence>
      {isOpen && (
        <motion.div
          ...
          className="absolute left-0 top-full mt-4 w-[650px] bg-white dark:bg-slate-900 shadow-[0_20px_40px_-15px_rgba(0,0,0,0.1)] dark:shadow-[0_20px_40px_-15px_rgba(0,0,0,0.4)] rounded-xl border border-gray-100 dark:border-slate-800 overflow-hidden z-50 origin-top-left flex"
        >
          {/* Menu Items */}
          <div className="w-2/3 p-6 grid grid-cols-2 gap-x-6 gap-y-4">
            ...
            {loading ? (
              ...
              >
                <div className="w-10 h-10 rounded-xl bg-emerald-100/50 dark:bg-slate-800 flex items-center justify-center mr-3 group-hover:bg-emerald-200/50 dark:group-hover:bg-emerald-800/50 transition-colors text-emerald-600 dark:text-emerald-400 font-bold text-lg">
                  {cat.name.charAt(0)}
                </div>
                <div>
